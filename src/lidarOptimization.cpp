...
SurfForwardCostFunction::SurfForwardCostFunction(Eigen::Vector3d curr_point_, Eigen::Vector3d plane_unit_norm_, double negative_OA_dot_norm_, double weight_) 
                                                        : curr_point(curr_point_), plane_unit_norm(plane_unit_norm_), negative_OA_dot_norm(negative_OA_dot_norm_), weight(weight_){

}

bool SurfForwardCostFunction::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    Eigen::Quaterniond q_w_curr(parameters[0][3], parameters[0][0], parameters[0][1], parameters[0][2]);
    Eigen::Vector3d t_w_curr(parameters[0][4], parameters[0][5], parameters[0][6]);
    Eigen::Vector3d point_w = q_w_curr * curr_point + t_w_curr;
    residuals[0] = (plane_unit_norm.dot(point_w) + negative_OA_dot_norm) * weight ;
// * (1+abs(plane_unit_norm[2])
    if(jacobians != NULL)
    {
        if(jacobians[0] != NULL)
        {
...
SurfbackCostFunction::SurfbackCostFunction(Eigen::Vector3d curr_point_, Eigen::Vector3d plane_unit_norm_, double negative_OA_dot_norm_, double weight_) 
                                                        : curr_point(curr_point_), plane_unit_norm(plane_unit_norm_), negative_OA_dot_norm(negative_OA_dot_norm_), weight(weight_){

}

bool SurfbackCostFunction::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    Eigen::Quaterniond q_w_curr(parameters[0][3], parameters[0][0], parameters[0][1], parameters[0][2]);
    Eigen::Vector3d t_w_curr(parameters[0][4], parameters[0][5], parameters[0][6]);
    ...
    if(jacobians != NULL)
    {
        if(jacobians[0] != NULL)
        {   
            Eigen::Vector3d mutable_curr_point = curr_point;
            Eigen::Matrix3d skew_point_w = skew(mutable_curr_point);
            // std::cout<<"skew_point_w:"<<skew_point_w<<std::endl;

            Eigen::Matrix<double, 3, 6> dp_by_se3;
            dp_by_se3.block<3,3>(0,0) = -skew_point_w;
            (dp_by_se3.block<3,3>(0, 3)).setIdentity();
            dp_by_se3 = -q_inverse.matrix() * dp_by_se3;
            Eigen::Map<Eigen::Matrix<double, 1, 7, Eigen::RowMajor> > J_se3(jacobians[0]);
            J_se3.setZero();
            J_se3.block<1,6>(0,0) = plane_unit_norm.transpose() * dp_by_se3;
